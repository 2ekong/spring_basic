...
public class SqlMapConfig {
	...
	static {
		...
		try {
			...
			//MyBatis Annotation 을 사용하는 경우
			Class[] mappers = {SqlMapperInter.class}; // 클래스를 쓸 땐 확장자를 준다.
			for (Class cl : mappers) {
				sqlSessionFactory.getConfiguration().addMapper(cl); //sqlSessionFactory는 configuration을 갖고 있다.
			}
		} catch (Exception e) {
