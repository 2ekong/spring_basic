...
public interface BoardRepository extends JpaRepository<Board, Integer> {
    ...
    // 검색용 JPQL
    @Query("select b from Board b where b.name like %?1%") // name 별 검색, 순서로 매핑
    Page<Board> searchLike1(Pageable pageable, String searchValue);

    @Query("select b from Board b where b.title like %:searchValue%") // title 별 검색, 이름으로 매핑
    ...
    //조회수 증가
    @Modifying(clearAutomatically = true)//연속성 컨텍스트
