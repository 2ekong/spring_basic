...
public class ListController {
    ...
    @GetMapping("list")
    public String listProcess(Model model, @RequestParam(required = false, defaultValue = "1", value = "page") int page) {
        // paging
        Page<Board> list = daoProcess.listAll(page - 1);

        int nowPage = list.getPageable().getPageNumber() + 1; // 1로 봐야하니까 + 1
        int startPage = Math.max(nowPage - 4, 1);
        int endPage = Math.min(nowPage + 5, list.getTotalPages()); // 최소 TotalPages까지만 나와야 함
//        Math.max(5, 3); // 5 반환
